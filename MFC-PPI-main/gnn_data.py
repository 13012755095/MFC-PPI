...
class GNN_DATA:
    ...
    # 这个方法要改，改为语言模型嵌入向量
    def get_feature_origin1(self, pseq_path, vec_path):
        self.get_protein_aac(pseq_path)
        ...
        for p_name in tqdm(self.pseq_dict.keys()):
            ...
            # 将下面改为直接通过氨基酸序列获取嵌入向量 seqvec protBert ELMo 等
            temp_vec = []
            for acid in temp_seq:
                temp_vec.append(self.acid2vec[acid])
            temp_vec = np.array(temp_vec)

            # 这个方法是将长度不够2000的氨基酸序列后面补0，超过2000的进行截取操作，看看是否还需要保留
            temp_vec = self.embed_normal(temp_vec, self.dim)
    ...
    def split_dataset(self, train_valid_index_path, test_size=0.2, random_new=False, mode='random'):
        if random_new:
            ...
            if mode == 'random':
                ...
                train_valid_index_timestamp_path = f'./train_val_split_data/train_val_split_{timestamp}.json'

                with open(train_valid_index_timestamp_path, 'w') as f:
                    f.write(jsobj)
