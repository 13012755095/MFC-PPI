...
from utils import Metrictor_PPI, print_file, Metrictor_PPI1

parser = argparse.ArgumentParser(description='MFC-PPI_model_training')

parser.add_argument('--ppi_path', default='./protein_info/protein.actions.SHS148k.txt', type=str,
                    help="ppi path")
parser.add_argument('--pseq_path', default='./protein_info/protein.SHS148k.sequences.dictionary.tsv', type=str,
                    help="protein sequence path")
parser.add_argument('--vec_path', default='./protein_info/vec5_CTC.txt', type=str,
                    help='protein sequence vector path')
# parser.add_argument('--p_feat_matrix', default='./protein_info/protein.nodes.SHS148k.D12.pt', type=str,
#                     help="protein feature matrix")
# parser.add_argument('--p_adj_matrix', default='./protein_info/protein.rball.edges.SHS148kD12.npy', type=str,
#                     help="protein adjacency matrix")
parser.add_argument('--p_feat_matrix', default='./protein_info/protein.nodes.SHS148k.pt', type=str,
                    help="protein feature matrix")
parser.add_argument('--p_adj_matrix', default='./protein_info/protein.rball.edges.SHS148k.npy', type=str,
                    help="protein adjacency matrix")
parser.add_argument('--index_path', default='./train_val_split_data/148k.random.fold1.json', type=str,
                    help='training and test PPI index')
parser.add_argument('--model_path', default='./result_save/28best/gnn_model_valid_best.ckpt', type=str,
...
def test(model, graph, test_mask, device, batch, p_x_all, p_edge_all, edge_num_index):
    ...
    for step in tqdm(range(valid_steps)):
        ...
        output = model(batch, p_x_all, p_edge_all, edge_num_index, graph.edge_index, valid_edge_id, graph.x)
    ...
    torch.save(true_prob_list, os.path.join("./result_save/fanhua/", f'epoch_outputs_test1dfs.pt'))
    torch.save(valid_label_list, os.path.join("./result_save/fanhua/", f'epoch_labels_test1dfs.pt'))
    # metrics = Metrictor_PPI(valid_pre_result_list, valid_label_list, true_prob_list)
    metrics = Metrictor_PPI1(valid_pre_result_list, valid_label_list)
...
def main():
    ...
    test(model, graph, graph.test1_mask, device, batch, p_x_all, p_edge_all, edge_num_index)
